...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        try {
            ...
            PreparedStatement check = conn.prepareStatement("SELECT * FROM institute WHERE email=? AND password=?");
            check.setString(1, email);
            check.setString(2, oldpass);
            ResultSet rs = check.executeQuery();

            if (rs.next()) {
                PreparedStatement update = conn.prepareStatement("UPDATE institute SET password=? WHERE email=?");
